...
int main()
{
    ...
    {
        auto entry = std::make_unique<BasicBlock>(ctx.getLabelTy(), "entry", addFnRaw);
    ...
    Instruction *cmpX = builder.CreateICmp(Instruction::ICmpPredicate::EQ, c1, c1);
    builder.CreateCondBr(cmpX, x_then.get(), x_else.get());

    builder.SetInsertPoint(x_then.get());
    Instruction *cmpY = builder.CreateICmp(Instruction::ICmpPredicate::EQ, c2, c2);
